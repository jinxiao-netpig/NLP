...
from module import LibMF, SCoR_Plus
...
parser.add_argument('--lp', type=float, default=0.5,
                    help='size of embeddings')
parser.add_argument('--lq', type=float, default=0.5,
                    help='size of embeddings')
# 训练模型
...
if args.model.lower() == 'scor+':
    model = SCoR_Plus(nuser, nitem, args.emsize).to(device)
    logger.log_message('Model: SCoR+')
else:
    model = LibMF(nuser, nitem, args.emsize).to(device)
    logger.log_message('Model: LibMF')
...
def train(data):
    ...
    while True:
        ...
        rating_p, user_src, item_src = model(user, item)  # (batch_size,)
        r_loss = loss_fn(rating_p, rating)
        reg_loss = args.lp * user_src.norm(dim=-1).mean() + args.lq * item_src.norm(dim=-1).mean()
        ...
        regularized_loss += batch_size * reg_loss.item()
        total_loss   += batch_size * loss.item()
        total_sample += batch_size

        # 打印损失
        if data.step % args.log_interval == 0 or data.step == data.total_step:
            cur_ra_loss = rating_loss / total_sample
            cur_re_loss = regularized_loss / total_sample
            cur_t_loss  = total_loss  / total_sample
            logger.log_message('rating loss {:4.4f} | regularized loss {:4.4f} | loss {:4.4f} | {:5d}/{:5d} batches'.format(
                 cur_ra_loss, cur_re_loss, cur_t_loss, data.step, data.total_step))
            rating_loss  = 0.
            total_loss   = 0.
            total_sample = 0
...
def evaluate(data):
    ...
    with torch.no_grad():
        while True:
            ...
            rating_p, user_src, item_src = model(user, item)  # (batch_size,)
            ra_loss = loss_fn(rating_p, rating)
            re_loss = args.lp * user_src.norm(dim=-1).mean() + args.lq * item_src.norm(dim=-1).mean()

            rating_loss      += batch_size * ra_loss.item()
            regularized_loss += batch_size * re_loss.item()
            total_sample += batch_size

            if data.step == data.total_step:
                break
    return rating_loss / total_sample, regularized_loss / total_sample
...
for epoch in range(1, args.epochs + 1):  # 循环遍历每个训练周期（epoch）
    ...
    ra_loss, re_loss = evaluate(val_data)  # 评估模型/计算损失（上下文、解释、评分）
    val_loss = ra_loss + re_loss
    logger.log_message('valid rating loss {:4.4f} | regularized loss {:4.4f} | loss {:4.4f} on validation'.format(ra_loss, re_loss, val_loss))
...
def generate(data):
    ...
    with torch.no_grad():
        while True:
            ...
            rating_p, _, _ = model(user, item)  # (batch_size,)
...
# Run on test Data.
test_ra_loss, test_re_loss = evaluate(test_data)
logger.log_message('=' * 89)
logger.log_message('rating loss {:4.4f} | regularized loss {:4.4f} on test | End of training'.format(test_ra_loss, test_re_loss))
generate(test_data)
