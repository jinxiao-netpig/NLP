...
class DataLoader:
    def __init__(self, data_path, index_dir, vocab_size):
        ...
        self.data, self.train, self.valid, self.test = self.load_data(data_path, index_dir)
...
class Batchify:
    def __init__(self, data, word2idx, seq_len=15, batch_size=128, shuffle=False):
        ...
        self.rating_i = torch.tensor(r, dtype=torch.int64).contiguous()
    ...
    def next_batch(self):
        ...
        seq     = self.seq[index]      # (batch_size, seq_len + 2)
        feature = self.feature[index]
        rating_i = self.rating_i[index]  # (batch_size, 1)
        return user, item, rating, seq, feature, rating_i
...
# use word as token(indefinite length)
