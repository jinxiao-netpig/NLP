...
from module3 import PETER_MF_R, PETER_SCoR_R
...
parser.add_argument('--rating_model', type=str, default='mf',
...
if args.rating_model == 'scor':
    ...
    if args.fixed_params:
        logger.log_message('Model: PETER-SCoR-R (Fixed Params)')
    else:
        logger.log_message('Model: PETER-SCoR-R')
else:
    model = PETER_MF_R(
        tgt_len, pad_idx,
        nuser, nitem, ntokens,
    ...
    if args.fixed_params:
        logger.log_message('Model: PETER-MF-R (Fixed Params)')
    else:
        logger.log_message('Model: PETER-MF-R')
...
def evaluate(data):
    ...
    with torch.no_grad():
        while True:
            user, item, rating, seq, _, rating_i = data.next_batch()  # (batch_size, seq_len), Data.step += 1
            batch_size = user.size(0)
            user   = user.to(device)    # (batch_size,)
            item   = item.to(device)
            rating = rating.to(device)
            seq    = seq.to(device)     # (batch_size, tgt_len + 1)
            rating_i = rating_i.to(device)
            # model.predict_seq(user, item, seq[:, :-1])
            rating_p, log_word_prob = model(user, item, rating_i, seq[:, :-1])  # (batch_size,) vs. (batch_size, tgt_len, ntoken)
...
def r_train(data):
    ...
    while True:
        ...
        rating_p, _ = model(user, item, _, _, seq_pred=False)  # (batch_size,) vs. (batch_size, tgt_len, ntoken)
...
def generate(data):
    ...
    with torch.no_grad():
        while True:
            user, item, rating, seq, _, rating_i = data.next_batch()
            user = user.to(device)  # (batch_size,)
            item = item.to(device)
            ...
            for idx in range(args.words):
                ...
                if idx == 0:
                    rating_p, log_word_prob = model(user, item, rating_i, text)  # (batch_size,) vs. (batch_size, 1, ntoken)
                    rating_predict.extend(rating_p.tolist())
                else:
                    log_word_prob = model.predict_seq(user, item, rating_i, text)  # (batch_size, text_len, ntoken)
