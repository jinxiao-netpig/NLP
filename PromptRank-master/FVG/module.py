...
class FVG(nn.Module):
    def __init__(self, tgt_len, pad_idx, nuser, nitem, ntoken, emsize, nhead, nlayers, dropout=0.2):
        ...
        self.decoder_layer = nn.TransformerDecoderLayer(
            d_model=emsize, nhead=nhead, batch_first=True, dropout=dropout)
        self.decoder = nn.TransformerDecoder(
            self.decoder_layer, num_layers=nlayers)
        self.fc_out = nn.Linear(emsize, ntoken)

        self.attn_mask = generate_square_subsequent_mask(tgt_len)
        self.emsize  = emsize
    ...
    def set_parameter_requires_grad(self, rating=True, text=True):
        ...
        self.decoder.requires_grad_(text)

    def get_rating_params(self):
        params = list(self.user_embeddings.weight) + list(self.item_embeddings.weight)
        return params
    ...
    def predict_seq(self, user, item, text):
        u_src = self.user_embeddings(user).unsqueeze(1)  # (batch_size, emsize) -> (batch_size, 1, emsize)
        i_src = self.item_embeddings(item).unsqueeze(1)  # (batch_size, emsize) -> (batch_size, 1, emsize)
        src = torch.cat(tensors=[u_src, i_src], dim=1)   # (batch_size, 2, emsize)
        src = src * math.sqrt(self.emsize)

        device = text.device
        tgt_len = text.shape[1]
        attn_mask = self.attn_mask[:tgt_len, :tgt_len].to(device)  # (tgt_len, tgt_len)
        key_padding_mask = text == self.pad_idx
        tgt = self.word_embeddings(text)  # (batch_size, tgt_len, emsize)
        tgt = tgt * math.sqrt(self.emsize)
        tgt = self.pos_encoder(tgt)
        memory = self.encoder(src)
        output = self.decoder(tgt, memory, tgt_mask=attn_mask, tgt_key_padding_mask=key_padding_mask)
        word_prob = self.fc_out(output)                      # (batch_size, tgt_len, ntoken)
        log_word_prob = func.log_softmax(word_prob, dim=-1)
        return log_word_prob

    def forward(self, user, item, text, rating_pred=True, seq_pred=True):
        if rating_pred:
            rating = self.predict_rating(user, item)            # (batch_size,)
        else:
            rating = None

        if seq_pred:
            log_word_prob = self.predict_seq(user, item, text)  # (batch_size, tgt_len, ntoken)
        else:
            log_word_prob = None
        return rating, log_word_prob
